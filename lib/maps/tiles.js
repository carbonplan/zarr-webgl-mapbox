...
export const createTiles = (regl, opts) => {
  ...
  function Tiles({
    ...
    const emptyTexture = ndarray(
      new Float32Array(Array(size * size).fill(-999)),
      [size, size]
    )
            ...
            Array(Math.pow(2, z))
              ...
              .map((_, y) => {
                ...
                this.variables.forEach(
                  (k) => (buffers[k + 'Attribute'] = regl.texture(emptyTexture))
                )
                this.tiles[key] = {
                  cached: false,
                  loading: false,
    ...
    this.drawTiles = regl({
      vert: `
        ...
        varying vec2 uv;
          ...
          float x = magnification * (position.y + offset.x * size) - globalMagnification * camera.x * size ;
          float y = magnification * (position.x + offset.y * size) - globalMagnification * camera.y * size ;
	        x = (scale * x);
	        y = (scale * y);
