...
export const createTiles = (regl, opts) => {
  ...
  function Tiles({
    ...
    this.ndim = ndim
    this.uniforms = Object.keys(customUniforms)
    ...
    let position = []
    let count, primitive, initialize, attributes, uniforms
    ...
    if (mode === 'grid' || mode === 'dotgrid') {
      ...
      attributes = { value: regl.prop('value') }
      uniforms = {}
    }
    ...
    if (mode === 'texture') {
      ...
      attributes = {}
      uniforms = { value: regl.prop('value') }
    }
          ...
          .map((_, x) => {
            Array(Math.pow(2, z))
              ...
              .map((_, y) => {
                ...
                const buffers = {}
                this.variables.forEach((k) => (buffers[k] = initialize()))
                this.tiles[key] = {
                  cached: false,
                  loading: false,
                  ready: false,
